...
/// Benchmarks the Deepfold backend for a given circuit size k and circuit type C.
fn bench_deepfold<C: CircuitExt<F>>(k: usize) {
    // Type alias: use our Deepfold PCS as the underlying polynomial commitment scheme.
    type DeepfoldPcs = Deepfold;
    // Define the backend that uses Deepfold PCS.
    type DeepfoldBackend = backend::hyperplonk::HyperPlonk<DeepfoldPcs>;
    ...
    // (4) Setup: produce universal parameters for DeepfoldBackend.
    let timer = start_timer(|| format!("deepfold_setup-{k}"));
    let param = DeepfoldBackend::setup(&circuit_info, std_rng()).unwrap();
    end_timer(timer);

    // (5) Preprocess: derive prover and verifier parameters.
    let timer = start_timer(|| format!("deepfold_preprocess-{k}"));
    let (pp, vp) = DeepfoldBackend::preprocess(&param, &circuit_info).unwrap();
    end_timer(timer);
...
/// Enum representing the proof systems we benchmark; here we support Deepfold.
...
impl System {
    ...
    fn bench(&self, k: usize, circuit: Circuit) {
        ...
        match self {
            System::Deepfold => match circuit {
                Circuit::VanillaPlonk => bench_deepfold::<VanillaPlonk<F>>(k),
                Circuit::Aggregation => {
                    // If you have an aggregator circuit for Deepfold, call its benchmark here.
                }
                Circuit::Sha256 => {
                    // If you have a SHA256 circuit benchmark for Deepfold, call it here.
                }
            },
        }
    }
    ...
    fn support(&self, circuit: Circuit) -> bool {
        matches!((self, circuit), (System::Deepfold, Circuit::VanillaPlonk))
    }
}
        ...
        |(mut systems, mut circuit, mut k_range), (key, value)| {
            match key.as_str() {
                "--system" => match value.as_str() {
                    ...
                    _ => panic!("System must be one of {{all, deepfold}}"),
                },
...
fn sample<T>(system: System, k: usize, prove: impl Fn() -> T) -> T {
    ...
    println!(
        "deepfold: k = {k}, avg proving time = {} ms",
        avg.as_millis()
    );
    proof.unwrap()
}
